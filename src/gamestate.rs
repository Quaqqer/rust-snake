...
impl GameState {
    pub fn tick(&mut self) -> TickResult {
        ...
        if self.fruit.is_some() && self.snake.pos == self.fruit.unwrap() {
            self.fruit = Some(self.find_empty_space(3.0));
    ...
    pub fn find_empty_space(&self, snake_dist: f32) -> Point2D {
        loop {
            ...
            if p.dist(self.snake.pos) < snake_dist {
                continue;
            }
    ...
    pub fn new(width: u32, height: u32) -> GameState {
        ...
        gs.fruit = Some(gs.find_empty_space(3.0));
